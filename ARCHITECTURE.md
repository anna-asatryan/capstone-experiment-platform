...
TAU   = C_FP / (C_FP + C_FN) = 0.1667
```

For any case with predicted default probability `p`:
- The cost-optimal decision is APPROVE iff `p < TAU`, otherwise REJECT.
- The realized cost on a single trial is:
  - `C_FN` if the participant approved and the borrower defaulted
  - `C_FP` if the participant rejected and the borrower would have repaid
  - `0` otherwise

`total_cost` and `optimal_cost` are summed across the 18 experimental trials
only (practice excluded). Both are written to `participants` at the end of the
performance screen.
...
Three design choices shape participant experience beyond the core experimental
manipulation. Each is motivated by the trade-off between data quality and
participant fatigue across 18 near-identically-structured trials.

### 7.1 No confidence slider
Earlier versions collected a 0–100 confidence rating per trial. This was
dropped for three reasons: (a) it was optional and therefore not usable as a
primary outcome, (b) the continuous probability estimate already provides
the information-rich signal needed for H3 and H5, and (c) each slider adds
~3 seconds of interaction cost across 20 trials (~1 min total), which compounds
with the 5-second review gate and other controls. The `confidence` column
is retained in the schema for backward compatibility but is NULL on new runs.

### 7.2 Deterministic template variation
Each loan description is rendered using one of three equivalent opening
templates, selected via `md5(participant_id, case_id) mod 3`. The
information content (amount, rate, term, purpose, income, DTI, utilization,
credit history, home ownership) is identical across variants; only the
surface phrasing differs. A given participant sees the same case with the
same phrasing on any reload, which preserves the resumption contract.
Different participants see different variants, which reduces the monotony
of reading 18 structurally identical cases.

### 7.3 Metacognitive reflection screen
Between trial 18 and the trust rating, participants answer two single-select
questions: (a) how often they changed their mind after seeing AI advice, and
(b) what they did when AI predictions surprised them. These self-reports are
compared against the behavioral reliance patterns computed from the trial
data, supporting exploratory analyses of metacognitive calibration — whether
participants who accurately describe their own reliance patterns also show
better probability calibration (H5-adjacent).
